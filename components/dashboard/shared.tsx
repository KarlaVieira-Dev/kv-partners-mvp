...
export function KPIGrid({ isLoading, metrics }: { isLoading?: boolean; metrics: KPIMetric[] }) {
  return (
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {metrics.slice(0, 4).map((metric) => (
        <article className="rounded-lg border border-white/10 bg-[#050810] p-5 shadow-sm" key={metric.label}>
  ...
  return (
    <section className="rounded-lg border border-white/10 bg-[#050810] shadow-sm">
        ...
        <table className="w-full border-collapse text-left" style={{ minWidth }}>
          <thead>
            <tr className="border-b border-white/10 bg-[#0c1120] text-xs font-medium uppercase tracking-[0.12em] text-zinc-400">
              {columns.map((column) => (
                <th className="px-5 py-3" key={column.header}>{column.header}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {rows.map((row, index) => (
              <tr className="align-top text-sm text-zinc-400 hover:bg-white/[0.02] transition" key={getRowKey(row, index)}>
                {columns.map((column) => (
                  <td className={cn("px-5 py-4", column.className)} key={column.header}>
                    {column.render(row)}
                  </td>
                ))}
              </tr>
      ...
      {!isLoading && rows.length === 0 ? (
        <div className="border-t border-white/10 px-5 py-10 text-center text-sm text-zinc-400">
          {emptyMessage}
        </div>
      ) : null}
    </section>
...
export function Pagination({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-white/10 bg-[#050810] px-4 py-3 text-sm text-zinc-400 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      <span>Página {normalizedPage} de {totalPages}</span>
      <div className="flex items-center gap-2">
        <button
          className="flex items-center gap-2 rounded-lg border border-white/10 bg-[#0c1120] px-3 py-1.5 text-sm font-medium text-zinc-400 transition hover:border-white/20 hover:text-white disabled:opacity-40 disabled:cursor-not-allowed"
...
export function IntelligentSummary({
  ...
  return (
    <section className="rounded-lg border border-blue-500/20 bg-[#050810] p-5 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-blue-400 mb-3">
        {title}
      </p>
      {meta ? (
        <div className="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 mb-5">
          {meta.map((item) => (
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        {items.map((item) => (
          <p className="rounded-lg border border-white/10 bg-[#0c1120] p-3 text-sm leading-6 text-zinc-300" key={item}>
            {item}
          </p>
        ))}
      </div>
...
export function usePaginatedRows<T>(items: T[], pageSize = 10) {
  ...
  return { page: Math.min(page, totalPages), paginatedRows, setPage };
}
